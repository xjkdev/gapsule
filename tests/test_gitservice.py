...
GitHTTPHandler.current_user = MagicMock(
    spec=property, __get__=lambda *args: 'abcd')
...
class GitServiceTestCase(AsyncHTTPTestCase):
    ...
    async def test_git_service(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            p1 = await asyncio.create_subprocess_exec('git', 'clone', url, cwd=tmpdir,
            ...
            with open(os.path.join(newrepo, filename), 'w') as f:
                f.write(content)
                f.close()
            p2 = await asyncio.create_subprocess_exec('git', 'add', filename, cwd=newrepo,
                                                      stdout=DEVNULL, stderr=DEVNULL)
            self.assertEqual(await p2.wait(), 0)
            p3 = await asyncio.create_subprocess_exec('git', 'commit', '-m', msg, cwd=newrepo,
                                                      stdout=DEVNULL, stderr=DEVNULL)
            self.assertEqual(await p3.wait(), 0)
            p4 = await asyncio.create_subprocess_exec('git', 'push', cwd=newrepo,
                                                      stdout=DEVNULL, stderr=DEVNULL)
